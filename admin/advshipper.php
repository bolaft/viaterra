 ...
 * @version    $Id: advshipper.php 313 2009-02-23 11:26:11Z Bob $
 */
...
<fieldset class="AdvancedShipperMethod<?php echo (($method_num % 2 == 0) ? 'Even' : 'Odd'); ?>">
	...
	if (strlen($method_admin_titles[$method_num][$_SESSION['languages_id']]) > 0) {
		echo ' - &ldquo;' . htmlentities($method_admin_titles[$method_num][$_SESSION['languages_id']]) . '&rdquo;';
	}
?></legend>
<?php
								...
								<table border="0" width="100%" cellpadding="0" cellspacing="0">
									<tr>
										...
										<td class="AdvancedShipperConfigField">
											<?php
											echo htmlentities($method_titles[$method_num][$_SESSION['languages_id']]);
											?>
										</td>
									</tr>
								</table>
						...
						<td>
							<fieldset width="100%" class="AdvancedShipperMethodSummary<?php echo (($method_num % 2 == 0) ? 'Even' : 'Odd'); ?>">
								...
								if (!$method_uses_fallover && $num_categories == 0 &&
									...
									echo '<p>' . htmlentities(TEXT_NO_CATEGORIES_MANUFACTURERS_PRODUCTS_SELECTIONS) . '</p>';
								}
								?>
							</fieldset>
						</td>
					...
					<tr>
						...
						?>
							<fieldset width="100%" class="AdvancedShipperMethodSummary<?php echo (($method_num % 2 == 0) ? 'Even' : 'Odd'); ?> AdvancedShipperMethod<?php echo (($method_num % 2 == 0) ? 'Even' : 'Odd') .
								...
								<table border="0" width="100%" cellpadding="0" cellspacing="0">
									<tr>
										<td class="AdvancedShipperConfigLabel"><label for="<?php echo "region_admin_titles[$region_num]"; ?>"><?php echo TEXT_LABEL_REGION_TITLE; ?></label></td>
										<td class="AdvancedShipperConfigField">
											<?php
											if (strlen($region_admin_titles[$region_num][$_SESSION['languages_id']]) > 0) {
												echo '&ldquo;'  . htmlentities($region_admin_titles[$region_num][$_SESSION['languages_id']]) . '&rdquo;';
											} else {
												echo '&nbsp;';
											}
											?>
										...
										<td class="AdvancedShipperConfigField">
											...
											if ($num_zones > 0) {
												foreach ($countries_zones_info as $country_zone) {
													echo '<p>' . htmlentities($country_zone['name']) . '</p>';
												}
											}
											?>
										</td>
										...
										<td class="AdvancedShipperConfigField">
											...
											if ($num_states > 0) {
												foreach ($countries_states_info as $country_state) {
													echo '<p>' . htmlentities(localities_get_level_2_locality_name($country_state['level_2_id'], null, true)) . '</p>';
												}
											}
											?>
										</td>
										...
										<td class="AdvancedShipperConfigField">
											...
											if ($num_cities > 0) {
												foreach ($countries_cities_info as $country_city) {
													echo '<p>' . htmlentities(localities_get_level_3_locality_name($country_city['level_3_id'], null, true)) . '</p>';
												}
											}
											?>
										</td>
									...
									<tr>
										...
										<td class="AdvancedShipperConfigField">
											<?php
											echo '<p>' . htmlentities($region_info[$region_i]['distance']) . '</p>';
											?>
										</td>
									</tr>
									<?php
									...
									<tr>
										...
										<td class="AdvancedShipperConfigField">
											...
											if (strlen(trim($region_info[$region_i]['table_of_rates'])) == 0) {
												...
												echo '<p>' . htmlentities(str_replace(',', ', ', $region_info[$region_i]['table_of_rates'])) . '</p>';
											}
											?>
										</td>
									</tr>
									<tr>
										...
										<td class="AdvancedShipperConfigField">
											...
											if (strlen(trim($region_info[$region_i]['surcharge'])) == 0) {
												...
												echo '<p>' . htmlentities(str_replace(',', ', ', $region_info[$region_i]['surcharge'])) . '</p>';
											}
											?>
										</td>
									</tr>
						...
						?>
							<fieldset width="100%" class="AdvancedShipperMethodSummary<?php echo (($method_num % 2 == 0) ? 'Even' : 'Odd'); ?>">
								...
								<p class="NoRegionsDefined"><?php echo htmlentities(TEXT_NO_REGIONS_DEFINED); ?></p>
							</fieldset>
						<?php
						}
						?>
