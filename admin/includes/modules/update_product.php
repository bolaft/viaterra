 ...
 */
  ...
  if (isset($_POST['edit_x']) || isset($_POST['edit_y'])) {
    ...
    $products_date_available = zen_db_prepare_input($product['products_date_available']);
    ...
    $tmp_value = zen_db_prepare_input($product['products_quantity']);

    $products_quantity = (!zen_not_null($tmp_value) || $tmp_value=='' || $tmp_value == 0) ? 0 : $tmp_value;

    $tmp_value = zen_db_prepare_input($product['products_price']);

    $products_price = (!zen_not_null($tmp_value) || $tmp_value=='' || $tmp_value == 0) ? 0 : $tmp_value;

    $tmp_value = zen_db_prepare_input($product['products_weight']);

    $products_weight = (!zen_not_null($tmp_value) || $tmp_value=='' || $tmp_value == 0) ? 0 : $tmp_value;

    $tmp_value = zen_db_prepare_input($product['manufacturers_id']);
    ...
    $sql_data_array = array('products_quantity' => $products_quantity,
                            ...
                            'products_status' => zen_db_prepare_input($product['products_status']),

                            'products_virtual' => zen_db_prepare_input($product['products_virtual']),

                            'products_tax_class_id' => zen_db_prepare_input($product['products_tax_class_id']),

                            'manufacturers_id' => $manufacturers_id,

                            'products_quantity_order_min' => zen_db_prepare_input($product['products_quantity_order_min']),

                            'products_quantity_order_units' => zen_db_prepare_input($product['products_quantity_order_units']),

                            'products_priced_by_attribute' => zen_db_prepare_input($product['products_priced_by_attribute']),

                            'product_is_free' => zen_db_prepare_input($product['product_is_free']),

                            'product_is_call' => zen_db_prepare_input($product['product_is_call']),

                            'products_quantity_mixed' => zen_db_prepare_input($product['products_quantity_mixed']),

                            'product_is_always_free_shipping' => zen_db_prepare_input($product['product_is_always_free_shipping']),

                            'products_qty_box_status' => zen_db_prepare_input($product['products_qty_box_status']),

                            'products_quantity_order_max' => zen_db_prepare_input($product['products_quantity_order_max']),

                            'products_sort_order' => (int)zen_db_prepare_input($product['products_sort_order']),

                            'products_discount_type' => zen_db_prepare_input($product['products_discount_type']),

                            'products_discount_type_from' => zen_db_prepare_input($product['products_discount_type_from']),

                            'products_price_sorter' => zen_db_prepare_input($product['products_price_sorter'])
    ...
    // when set to none remove from database
      ...
      $sql_data_array['products_image'] = zen_db_prepare_input($product['products_image']);

      $new_image= 'true';



    if ($product['image_delete'] == 1) {
    ...
    if ($action == 'insert_product') {
      ...
      $update_sql_data = array( 'products_last_modified' => 'now()',
                                ...
                                'master_categories_id' => ($product['master_category'] > 0 ? zen_db_prepare_input($product['master_category']) : zen_db_prepare_input($product['master_categories_id'])));
    ...
    for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
      ...
      $sql_data_array = array('products_name' => zen_db_prepare_input($product['products_name'][$language_id]),

                              'products_description' => zen_db_prepare_input($product['products_description'][$language_id]),

                              'products_url' => zen_db_prepare_input($product['products_url'][$language_id]));
    ...
    for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
      ...
      $sql_data_array = array('metatags_title' => zen_db_prepare_input($product['metatags_title'][$language_id]),

                              'metatags_keywords' => zen_db_prepare_input($product['metatags_keywords'][$language_id]),

                              'metatags_description' => zen_db_prepare_input($product['metatags_description'][$language_id]));
