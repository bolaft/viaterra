...
*Modification du fichier pour pouvoir prévisualiser plusieurs produits
...
//auto replace with defined missing image
            ...
            if ($_POST['products_image_manual'] != '') {
              ...
              $products_image_name[$xxx] = $_POST['img_dir'] . $_POST['products_image_manual'];
            ...
            if ($_POST['image_delete'] == 1 || $products_image_name == '' and PRODUCTS_IMAGE_NO_IMAGE_STATUS == '1') {
              ...
              if (is_array($products_image_name)) {
                echo zen_image(DIR_WS_CATALOG_IMAGES . $products_image_name[$xxx], $pInfo->products_name, SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT, 'align="right" hspace="5" vspace="5"') . $pInfo->products_description;  
              }else
              {
...

      {
        ...
        foreach ($products as $product) {
          ...
          for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
            ...
            echo zen_draw_hidden_field($j.'-products_name[' . $languages[$i]['id'] . ']', htmlspecialchars(stripslashes($product['products_name'][$languages[$i]['id']])));

            echo zen_draw_hidden_field($j.'-products_description[' . $languages[$i]['id'] . ']', htmlspecialchars(stripslashes($product['products_description'][$languages[$i]['id']])));

            echo zen_draw_hidden_field($j.'-products_url[' . $languages[$i]['id'] . ']', htmlspecialchars(stripslashes($product['products_url'][$languages[$i]['id']])));

          }

          echo zen_draw_hidden_field($j.'-products_image', stripslashes($products_image_name[$j-1]));
