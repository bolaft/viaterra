 ...
 * @version    $Id: advshipper_method_config.php 313 2009-02-23 11:26:11Z Bob $
 */
	...
	if (strlen($regions_info) > 0) {
		...
		for ($region_i = 0; $region_i < $num_regions; $region_i++) {
			...
			$regions[$region_num]['table_of_rates'] =
				str_replace('--plus--', '+', urldecode($region_info[11]));
			$regions[$region_num]['max_weight_per_package'] = $region_info[12];
			$regions[$region_num]['packaging_weights'] =
				str_replace('--plus--', '+', urldecode($region_info[13]));
			$regions[$region_num]['surcharge'] =
				str_replace('--plus--', '+', urldecode($region_info[14]));
			
			// Decode surcharge titles
			$titles_encoded = urldecode($region_info[15]);
			...
			$ups_calc_string = $region_info[16];
			...
			$usps_calc_string = $region_info[17];
	...
	if (!$advshipper_demo && sizeof($errors) == 0) {
		...
		for ($region_i = 0; $region_i < $num_regions; $region_i++) {
			...
			$region_config_data_array = array(
				...
				'table_of_rates' => $regions[$region_num]['table_of_rates'],
				'max_weight_per_package' => ((is_null($regions[$region_num]['max_weight_per_package']) || strlen(trim($regions[$region_num]['max_weight_per_package'])) == 0) ? 'null' : $regions[$region_num]['max_weight_per_package']),
				'packaging_weights' => ((is_null($regions[$region_num]['packaging_weights']) || strlen(trim($regions[$region_num]['packaging_weights'])) == 0) ? 'null' : $regions[$region_num]['packaging_weights']),
				'surcharge' => ((is_null($regions[$region_num]['surcharge']) || strlen(trim($regions[$region_num]['surcharge'])) == 0) ? 'null' : $regions[$region_num]['surcharge'])
				);
			...
			for ($language_i = 0; $language_i < $num_languages; $language_i++) {
				$region_admin_titles_data_array = array(
					...
					'region' => $region_num,
					'language_id' => $languages[$language_i]['id'],
					'title' => (strlen($regions[$region_num]['titles'][$languages[$language_i]['id']]) == 0 ? 'null' : zen_db_prepare_input($regions[$region_num]['admin_titles'][$languages[$language_i]['id']]))
					);
				...
				$region_titles_data_array = array(
					...
					'region' => $region_num,
					'language_id' => $languages[$language_i]['id'],
					'title' => (strlen($regions[$region_num]['admin_titles'][$languages[$language_i]['id']]) == 0 ? 'null' : zen_db_prepare_input($regions[$region_num]['titles'][$languages[$language_i]['id']]))
					);
...
<fieldset class="AdvancedShipperMethod<?php echo ((($method_num + 1) % 2 == 0) ? 'Odd' : 'Even'); ?>">
	...
	if (strlen($method_admin_titles[$_SESSION['languages_id']]) > 0) {
		echo ' - &ldquo;' . htmlentities($method_admin_titles[$_SESSION['languages_id']]) . '&rdquo;';
	}
?></legend>
...
// -->
								...
								for ($category_i = 0; $category_i < $num_categories; $category_i++) {
									...
									echo '<p id="category_name_' . $categories[$category_i]['id'] . '">' . htmlentities($categories[$category_i]['name']);
...
// -->
								...
								for ($manufacturer_i = 0; $manufacturer_i < $num_manufacturers; $manufacturer_i++) {
									...
									echo '<p id="manufacturer_name_' . $manufacturers[$manufacturer_i]['id'] . '">' . htmlentities($manufacturers[$manufacturer_i]['name']);
...
// -->
								...
								<?php
								for ($product_i = 0; $product_i < $num_products; $product_i++) {
									echo '<p id="product_name_' . $products[$product_i]['id'] . '">' . htmlentities($products[$product_i]['name']);
